...
vector<pair<ll, vi>> klp (int s, int t, int k) {
	...
	// Si existen dichas aristas, hay que encontrar un orden topológico tal que p[u] aparezca
	// en el vector q antes que u. 
	...
	for (int u : q) {
		...
		if (p[u] != -1) pq[u] = pq[p[u]];
		
		int f = 1;
		
		for (ii &v : adj[u]) {
			if (dis[v.fi] == -1) continue;
	...
	if (dis[s] == -1) return vector<pair<ll, vi>>(k, {-1, {}});
	...
	if (pq[s]) hp.push({{dis[s] + pq[s]->val.fi, 0}, pq[s]});
	...
	while (k && hp.size()) {
		...
		int v = t->val.se.se;
		if (pq[v]) hp.push({{d + pq[v]->val.fi, new_st}, pq[v]});
		
		d -= t->val.fi;
		t = pop(t);
		if (t) hp.push({{d + t->val.fi, st}, t});
	}
