...
struct DSURollback{
    ...
    struct DataChanges{
        ...
        DataChanges(int u, int size_u, int v, int dad_v): u(u), size_u(size_u), v(v), dad_v(dad_v) {}
    };
    ...
    void rollback() {
        while(!changes.empty() && !changes.top().checkpoint) {
            DataChanges D = changes.top(); changes.pop();
        ...
        if(SZ(changes) != 1 && changes.top().checkpoint) changes.pop();
    }
