/*
    ...
    If relevant_nodes has k nodes, virtual Tree will have at most 2*k-1 total
   nodes.
*/
...
struct VirtualTreeMaker {
  ...
  template <class G> void build(const G &adjList, int root = 0) {
    dfs_prec(root, root, adjList);
  }

  template <class G> void dfs_prec(int u, int p, const G &adjList) {
  ...
  int virtual_tree(vector<int> nodes) {
    sort(all(nodes), [&](auto u, auto v) { return rang[u][0] < rang[v][0]; });
