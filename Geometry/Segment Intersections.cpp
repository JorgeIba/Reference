...
struct Event {
    ...
    bool operator<(const Event &e) const {
        if (s.p1.x != e.s.p1.x)
            return s.p1.x < e.s.p1.x;

        //! Mucho ojo con esto, depende de que se considera interseccion y qué
        //! no
        if (start && horizontal)
...
bool segmentIntersections(const vector<Segment> &segments) {
    ...
    for (auto e : events) {
        if (e.horizontal) {
            ...
            if (it_lowest == points_in_y.end())
                continue;

            if (it_lowest->second <=
                p_high.y) { // At least one point that intersects
