template <typename T> struct Poly {
    ...
    Poly operator*(const Poly<T> &A) { return multiply(A.P, P); }
    ...
    void buildSTE(vector<T> &x) { //(x-x_0)(x-x_1)(x-x_2)...(x-x_n)
        ...
        STE.resize(2 * n);
        for (int i = n; i < 2 * n; i++)
            STE[i] = vector<T>{sub(0, x[i - n]), 1}; // Ojo: MOD
        for (int i = n - 1; i; i--)
            STE[i] = STE[i << 1] * STE[i << 1 | 1];
    }
    ...
    Poly interpolate(vector<T> &x, vector<T> &y) {
        ...
        for (int i = n - 1; i; i--)
            inTree[i] = inTree[i << 1] * STE[i << 1 | 1] +
                        inTree[i << 1 | 1] * STE[i << 1];
        return inTree[1];
    }
...
template <typename T> ostream &operator<<(ostream &so, const Poly<T> &A) {
    for (auto x : A.P)
        so << x << " ";
    return so;
}