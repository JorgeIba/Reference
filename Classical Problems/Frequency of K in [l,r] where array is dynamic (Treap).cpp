...
template <typename T> struct TreapNode {
    ...
    TreapNode(T value = 0)
        : value(value), key(rand()), size(1), left(0), right(0), parent(0),
          original(0) {}
    TreapNode(TreapNode<T> *ori, int idx)
        : value(ori->value), key(rand()), size(1), left(0), right(0), parent(0),
          original(ori), idx(idx) {}
};

template <typename T = TreapNode<int>> struct Treap {
    T *root;
    ...
    static inline int aleatorio() { return (rand() << 15) + rand(); }
...
int main() {
    ...
    auto update = [&](int l, int r) -> void {
        ...
        TNode *left_k = NULL, *middle_k = NULL, *single_k = NULL,
              *right_k = NULL;
