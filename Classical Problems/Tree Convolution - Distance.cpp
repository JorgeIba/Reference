...
struct TreeConvolution{
    ...
    TreeConvolution(int n): n(n) , visited(n), sizeSub(n), heightSub(n), ansArray(2*n) {}
    ...
    lli getAns(F f, G g, H h){

        auto dfs = [&](const auto &dfs, int u, int dist, Poly &A, Poly &B, int p = -1) -> void{
            A[dist] += f(u);
            if(A[dist] >= MOD) A[dist] -= MOD;

            B[dist] += g(u);
            if(B[dist] >= MOD) B[dist] -= MOD;
