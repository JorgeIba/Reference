 ...
 * Sample use
 * vi a;
 * WaveletTree wv( a.begin(), a.end(), *min_element(a.begin(), a.end()),
 * *max_element(a.begin(),a.end()) ); 1-indexed
 */
struct WaveletTree {
  int low, high;
  ...
  WaveletTree(vi::iterator from, vi::iterator to, int x, int y)
      : low(x), high(y) {
    if (from >= to)
      return;
    int m = (low + high) / 2;
  ...
  // returns the number of elements lesser than or equal to x int then
  // subarray [l,..,r]
  int lessThanOrEqual(int l, int r, int x) {
    ...
    izq = mapLeft[l - 1];
    der = mapLeft[r];
    return left->lessThanOrEqual(izq + 1, der, x) +
           right->lessThanOrEqual(l - izq, r - der, x);
  }
  ...
  // counts the elements in the range[l,r] such that its value is in the range
  // [a,b]
  int elementsInRange(int l, int r, int a, int b) {
    ...
    if (a <= low && b >= high)
      return r - l + 1;
    int lb = mapLeft[l - 1], rb = mapLeft[r];
    return left->elementsInRange(lb + 1, rb, a, b) +
           right->elementsInRange(l - lb, r - rb, a, b);
  }
