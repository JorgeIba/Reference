...
struct Node{
	...
	Node *left, *right;
	Node(int key, int priority): key(key), priority(priority), value(key), left(NULL), right(NULL){}
};
...
struct Node{
	...
	Node *left, *right; 
	Node(int key, int priority): key(key), priority(priority), size(1), /*value(key),*/ left(NULL), right(NULL){}
};
...
//inserts a new node with key = x in the position ind of the treap T, returns the new root
