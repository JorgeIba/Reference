template <typename T> struct Matrix {
  ...
  Matrix(lli n, lli m)
      : rows(n), cols(m) // Matrix of n rows and m cols
  {
  ...
  void GaussJordan(bool SLE = false,
                   Matrix<T> &ans = NULL) // if inverse, ans = Identity
  {
    for (int row = 0, col = 0; row < rows && col < cols; col++) {
      if (A[row][col] == 0) {
        ...
        for (int i = row + 1; i < rows; i++) {
          if (abs(A[i][col] >
                  abs(A[pivot][col]))) { //* Selecciona la Pivot como el
                                         // numero mas grande (Heuristico)
            pivot = i;
          }
        }
        if (abs(A[pivot][col]) > 0) { //* Si el numero mas grande no es
                                      // cero, swapea las dos filas
          swapRows(pivot, row);
          if (SLE)
            ans.swapRows(pivot, row);
        } else
          continue;
      }
      ...
      for (int i = 0; i < rows;
           i++) //* Se realizan combinaciones lineales para que los otros
                // numeros sean 0 excepto el pivote
      {
        if (i != row &&
            A[i][col] != 0) //* No se agrega a el mismo ni cuando es 0 ya
        {
          T invSum = inverseSum(A[i][col]);
          addRow(i, row, invSum);
          if (SLE)
            ans.addRow(i, row, invSum);
        }
      }
      row++;
    }
...
template <typename T>
Matrix<T>
geometric(const Matrix<T> &M,
          int k) // A + A^2 + ... + A^(2*n) = (A+A^2+..+A^(n)) * (I + A^(n))
