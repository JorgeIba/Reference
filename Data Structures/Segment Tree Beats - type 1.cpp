...
template <typename T> struct SegmentTreeBeats {
  lli N;
  Node ST[4 * maxN];
  ...
  void updateNodeMAX(int curr, T value) {
    if (value > ST[curr].min) {
      if (ST[curr].max == ST[curr].min) {
        ...
        if (ST[curr].secondMax == ST[curr].min)
          ST[curr].secondMax = value;
  ...
  void updateNodeMIN(int curr, T value) {
    if (value < ST[curr].max) {
      if (ST[curr].max == ST[curr].min) {
        ...
        if (ST[curr].secondMin == ST[curr].max)
          ST[curr].secondMin = value;
  ...
  void updateNodePLUS(int curr, T value) {
    ST[curr].max += value;
    if (ST[curr].secondMax != -INF)
      ST[curr].secondMax += value;

    ST[curr].min += value;
    if (ST[curr].secondMin != INF)
      ST[curr].secondMin += value;
  ...
  void updateFromChildren(int curr) {
    ...
    ST[curr].secondMax =
        max(ST[2 * curr].secondMax, ST[2 * curr + 1].secondMax);
    ST[curr].frMax = 0;
    ...
    ST[curr].secondMin =
        min(ST[2 * curr].secondMin, ST[2 * curr + 1].secondMin);
    ST[curr].frMin = 0;
  ...
  bool breakCondition(int curr, int type_query, T value) {
    switch (type_query) {
    case MIN:
      return ST[curr].max < value;
    case MAX:
      return ST[curr].min > value;
  ...
  bool tagCondition(int curr, int type_query, T value) {
    switch (type_query) {
    case MIN:
      return ST[curr].secondMax < value;
    case MAX:
      return ST[curr].secondMin > value;
  ...
  void updateMIN(int curr, int l, int r, int ql, int qr, T value) {
    if (l > qr || r < ql || breakCondition(curr, MIN, value))
      return;
  ...
  void updateMIN(int ql, int qr, T value) { updateMIN(1, 1, N, ql, qr, value); }

  // MAX
  void updateMAX(int curr, int l, int r, int ql, int qr, T value) {
    if (l > qr || r < ql || breakCondition(curr, MAX, value))
      return;
  ...
  void updateMAX(int ql, int qr, T value) { updateMAX(1, 1, N, ql, qr, value); }

  // PlUS
  void updatePLUS(int curr, int l, int r, int ql, int qr, T value) {
    if (l > qr || r < ql || breakCondition(curr, PLUS, value))
      return;
  ...
  lli query_sum(int curr, int l, int r, int ql, int qr) {
    if (l > qr || r < ql)
      ...
      pushToChildren(curr);
      lli mid = (l + r) / 2;
      return query_sum(2 * curr, l, mid, ql, qr) +
             query_sum(2 * curr + 1, mid + 1, r, ql, qr);
    }
  }
  lli query_sum(int ql, int qr) { return query_sum(1, 1, N, ql, qr); }
  ...
  /*
  void debug(int curr){
  cout << "max: " << ST[curr].max  << ", sum: " << ST[curr].sum << " , frMax:
  " << ST[curr].frMax << " , secondMax: " << ST[curr].secondMax << ", min: "
  << ST[curr].min << ", secondMin: " << ST[curr].secondMin; cout << ", frMin:
  " << ST[curr].frMin << ", lazySum: " << ST[curr].lazySUM << endl;;
  }
