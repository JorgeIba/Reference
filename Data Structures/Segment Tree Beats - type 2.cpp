...
template <typename T> struct SegmentTreeBeats {
    ...
    void updateFromChildren(int curr) {
        ...
        bool sameRangeBoth =
            ST[2 * curr].equalRange && ST[2 * curr + 1].equalRange;
    ...
    void updateFLOOR(int curr, int l, int r, int ql, int qr, T value) {
        if (l > qr || r < ql || breakCondition(curr, FLOOR, value))
            return;
    ...
    /*
    void debug(int curr){
        cout << "Value: " << ST[curr].value << ", equalRange: " <<
    ST[curr].equalRange << ", lazyFLOOR: " << ST[curr].lazyFLOOR << ",
    lazyEQUAL: " << ST[curr].lazyEQUAL << endl;
    }
