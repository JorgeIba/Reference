struct Node{
    ...
    void extend(){
        ...
        if(!left_node) left_node = new Node(l_range, mid);
        if(!right_node) right_node = new Node(mid+1, r_range);
    }
...
void update(Node *curr, int left_query, int right_query, lli value){
    ...
    if(curr->l_range > right_query || curr->r_range < left_query) return;
    ...
    curr->value_range = curr->left_node->value_range + curr->right_node->value_range; // merge
}

lli query(Node *curr, int left_query, int right_query){
    // curr->apply_lazy(); // FOR LAZY

    if(curr->l_range > right_query || curr->r_range < left_query) return 0;
    else if( left_query <= curr->l_range && curr->r_range <=  right_query){
        return curr->value_range;
    }
    if(!curr->left_node || !curr->right_node) return 0; // No changes here
    return query(curr->left_node, left_query, right_query) + query(curr->right_node, left_query, right_query); // merge
}
